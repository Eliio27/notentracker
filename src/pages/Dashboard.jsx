...
export default function Dashboard({ onOpenTest, semesters, subjects, activeSemester }) {
  ...
  if (semesters.length === 0) return (
    <>
      <h1>Übersicht</h1>
      <div className="empty">Noch kein Semester erstellt.<br />Gehe zu "Verwalten".</div>
    </>
  )
  ...
  return (
    <>
      <div className="semester-switch">
        {semesters.map((s) => (
          <button key={s.id} className={`sem-chip ${s.id === viewSemId ? 'active' : ''}`}
      ...
      <div className="card" style={{ textAlign: 'center' }}>
        ...
        <div className="grade-big" style={{ color: allGrades.length ? gradeColor(totalAvg) : 'var(--text2)' }}>
          {allGrades.length ? totalAvg.toFixed(2) : '–'}
        </div>
        <div style={{ color: 'var(--text2)', fontSize: 12 }}>
          {tests.length} {tests.length === 1 ? 'Test' : 'Tests'} · {semSubjects.length} {semSubjects.length === 1 ? 'Fach' : 'Fächer'}
        </div>
      </div>
            ...
            return (
              <div key={t.id} className="test-card" onClick={() => onOpenTest(t.id)}>
                ...
                <div className="test-info">
                  ...
                  <div className="test-sub">{subjectMap[t.subjectId]?.name} · {formatDate(t.date)}</div>
                </div>
      ...
      {semSubjects.length === 0 && (
        <div className="empty">Keine Fächer in diesem Semester.<br />Erstelle Fächer unter "Verwalten".</div>
      )}
      {semSubjects.length > 0 && tests.length === 0 && (
        <div className="empty">Noch keine Tests in diesem Semester.</div>
      )}
    </>
  )
}
